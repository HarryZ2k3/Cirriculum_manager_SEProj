...
// const WeekView = () => {
//   // Find the unique weeks in the sampleClasses data
//   const uniqueWeeks = [...new Set(sampleClasses.map(event => moment(event.date).startOf('week').format()))];

//   // For simplicity, we'll display the first week in the sample data
//   const startOfWeek = moment(uniqueWeeks[0]);
//   const days = [];
//   for (let i = 0; i < 7; i++) {
//     days.push(startOfWeek.clone().add(i, 'days'));
//   }
...
const WeekView = ({ currentDate }) => {
  // Use the start of the current date's week for the display
      ...
      <div className="grid grid-cols-7 gap-1">
        ...
        {days.map((day, dayIndex) => (
          <div key={dayIndex} className="border p-2 h-auto relative bg-gray-50">
            ...
            <div className="space-y-2">
              {sampleClasses
                .filter(event => moment(event.date).isSame(day, 'day')) // Check if the event date matches the current day
                .map((event, eventIndex) => (
                  <div
                    key={eventIndex}
                    className="bg-blue-500 text-white p-2 rounded-lg shadow-md text-xs mb-2"
