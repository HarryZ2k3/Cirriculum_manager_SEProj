...
const Schedule = () => {
  ...
  const events = sampleClasses
    .filter(cls => cls.semester === currentSemester)
    .map(cls => {
      const start = moment(`2024-06-01T${cls.startTime}`).toDate();
      const end = moment(`2024-06-01T${cls.endTime}`).toDate();
      return {
        title: cls.name,
        start,
        end,
  ...
  const CustomTimeSlotWrapper = ({ children, value }) => {
    const periodIndex = moment(value).hours() * 2 + moment(value).minutes() / 30;
    const periodNames = [
    ...
    return (
      <div className="rbc-time-slot">
        {periodNames[periodIndex] || children}
      </div>
    );
  };
              ...
              >
                ...
                {hoveredEvent && hoveredEvent === event && (
                  ...
                  >
                    ...
                    <p><strong>Classroom:</strong> {event.resource.classroom}</p>
                    <p><strong>Time:</strong> {event.resource.startTime} - {event.resource.endTime}</p>
                    <p><strong>Period:</strong> {event.resource.period}</p>
                  </div>
                )}
              </div>
            )
