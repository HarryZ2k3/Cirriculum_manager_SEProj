...
const getGrades = `SELECT g.Inclass, g.Midterm, g.Final, (0.30 * g.Inclass + 0.30 * g.Midterm + 0.40 * g.Final) AS TotalGrade FROM 
        STUDENT.Grades g JOIN  COURSES.InforList c ON g.CourseID = c.CourseID JOIN 
        STUDENT.InforList s ON g.StudentID = s.StudentID JOIN ACCOUNT.StudentAccounts d ON s.SAID = d.SAID JOIN SEMESTERS.InforList si ON g.SemesterID = si.SemesterID WHERE d.Username = $1 AND si.SemesterNumber = $2 AND si.Year = $3`;
